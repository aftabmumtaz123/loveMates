...
export default function MomentDetail(){
 ...
 const [memory,setMemory]=useState<Memory|null>(null),[loading,setLoading]=useState(true),[error,setError]=useState(''),[commentText,setCommentText]=useState(''),[showReactions,setShowReactions]=useState(false),[busy,setBusy]=useState(false);
 ...
 const counts=memory.reactionCounts||{}; const total=Object.values(counts).reduce((a,b)=>a+b,0); const creatorId=String(memory.createdBy); const isCreator=user?.id===creatorId;
 return <div>
   ...
   <div className="overflow-hidden rounded-[2rem] border border-rose-100 bg-white shadow-[0_24px_70px_rgba(244,63,94,.10)] lg:h-[calc(100vh-9rem)] lg:min-h-[620px]">
    <div className="grid h-full lg:grid-cols-[minmax(0,1.35fr)_minmax(360px,.85fr)]">
      <section className="relative flex min-h-[360px] items-center justify-center overflow-hidden bg-slate-950">
        {memory.imageUrl?<img src={memory.imageUrl} className="h-full max-h-[calc(100vh-9rem)] w-full object-contain"/>:<div className="flex h-full min-h-[360px] w-full items-center justify-center bg-gradient-to-br from-rose-50 via-white to-violet-50 text-7xl">{memory.emoji}</div>}
